...
Respecto a la covarianza, $Cov(X_t,X_{t-k})$ para $K=1,...$

$$
\begin{split}
Cov(X_t,X_{t-k})&=\mathbb{E}\left[(X_t-\mathbb{E}(X_t))(X_{t-k}-\mathbb{E}(X_{t-k}))\right]\\
...
A continuación se da muestran los resultados para un modelo $AR(1)$ de la siguiente forma $X_t=0.35X_{t-1}+\epsilon_t$
...
$$
\begin{split}
\mathbb{E}[X_t]& =\mathbb{E}[\phi_1 X_{t-1}+\phi_2 X_{t-2}+\epsilon_t]\\
\implies &\mathbb{E}[(1-\phi_1B-\phi_2B^2) X_{t}]=\mathbb{E}[\epsilon_t]\\
\implies &(1-\phi_1B-\phi_2B^2) \mathbb{E}[X_{t}]=0\\
\implies & \mathbb{E}[X_{t}]=\frac{0}{1-\phi_1B-\phi_2B^2}
\end{split}
$$
...
$$
\begin{split}
\gamma_0 &= E\left[(X_{t}-E(X_{t})^2\right]=E[X_{t}]^2\\
& =E[\phi_1 X_{t-1}+\phi_2 X_{t-2}+\epsilon_t]^2
\gamma_0 &= \phi_1^2\gamma_0 +\phi_2^2\gamma_0 +\sigma^2+2\phi_1^2\phi_2^2\gamma_1\\
\implies & (1-\phi_1^2-\phi_2^2)\gamma_0 = \sigma^2+2\phi_1^2\phi_2^2\gamma_1\\
\gamma_0 &= \frac{\sigma^2+2\phi_1^2\phi_2^2\gamma_1}{1-\phi_1^2-\phi_2^2}\\
\end{split}
$$
...
&= E[(\phi_1 X_{t-1}+\phi_2 X_{t-2}+\epsilon_t)X_{t-1}]\\
&= \phi_1 E[X_{t-1}]^2+\phi_2 E[X_{t-2}X_{t-1}]+E[\epsilon_tX_{t-1}]\\
&= \phi_1\gamma_0 +\phi_2 \gamma_1\\
&= \frac{\phi_1\gamma_0} {1-\phi_2}
\end{split}
$$
...
$$
\begin{split}
\gamma_k &= E(X_{t}-E[X_{t}])(X_{t-k}-E[X_{t-k}])=E[X_{t}X_{t-k}]\\
 &= E[(\phi_1 X_{t-1}+\phi_2 X_{t-2}+\epsilon_t)X_{t-k}]\\
...
& = \mathbb{E}\left[\phi_1^2(X_{t-1}-\mathbb{E}(X_{t-1}))^2 + 2\phi_1\phi_2(X_{t-1}-\mathbb{E}(X_{t-1}))(X_{t-2}-\mathbb{E}(X_{t-2})) + \phi_2^2(X_{t-2}-\mathbb{E}(X_{t-2}))^2+\epsilon_0^2+\cdots\right]\\
